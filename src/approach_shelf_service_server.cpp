...
public:
    approachShelfServer() : Node("approach_shelf_server"){
        srv_approach_ = create_service<approachShelfSrv>("/approach_shelf_server", std::bind(&approachShelfServer::approachShelfCallback,this,_1,_2));
        ...
        distance_approach_target_error_ = this->get_parameter("distance_approach_target_error").as_double();

        this->declare_parameter("angle_approach_target_error",0.5);
        angle_approach_target_error_ = this->get_parameter("angle_approach_target_error").as_double();
        ...
        RCLCPP_INFO(this->get_logger(), "Servidor de servicio [approach_shelf_server] inicializado ");

        RCLCPP_INFO(this->get_logger(), "Vel min lin x [%.3f] ",vel_min_linear_x_);
        RCLCPP_INFO(this->get_logger(), "Vel min angular z [%.3f] ",vel_min_angular_z_);
        RCLCPP_INFO(this->get_logger(), "Vel max lin x [%.3f] ",vel_max_linear_x_);
        RCLCPP_INFO(this->get_logger(), "Vel max angular z [%.3f] ",vel_max_angular_z_);
        
        RCLCPP_INFO(this->get_logger(), "Kp lineal [%.3f] ",kp_lineal_);
        RCLCPP_INFO(this->get_logger(), "Kp angular [%.3f] ",kp_angular_);

        RCLCPP_INFO(this->get_logger(), "Distance to target_goal error [%.3f] ",distance_approach_target_error_);
        RCLCPP_INFO(this->get_logger(), "Angle to target_goal error [%.3f] ",angle_approach_target_error_);
        RCLCPP_INFO(this->get_logger(), "Laser min range [%.3f] ",laser_min_range_);
    }
...
private:
    ...
    void approachShelfCallback(const std::shared_ptr<approachShelfSrv::Request> request, const std::shared_ptr<approachShelfSrv::Response> response){
    ...
    void getTransform(std::string frame_head,std::string frame_child ){
        ...
        while(t.transform.translation.z  == std::numeric_limits<double>::max() && rclcpp::ok()){
            try {
                t = tf_buffer_->lookupTransform(frame_head, frame_child,tf2::TimePointZero);

            }catch (tf2::TransformException &ex) {
                RCLCPP_ERROR(this->get_logger(),"Llame a lookupTransform");
                RCLCPP_ERROR(this->get_logger(),"Error al obtener la transformacion de [%s]  a [%s]", frame_head.c_str(),frame_child.c_str());
    ...
    void calculateErrors(){
        ...
        distance_target = sqrt(std::pow(t.transform.translation.x,2) + std::pow(t.transform.translation.y,2));
        theta_target = std::atan2(t.transform.translation.y,t.transform.translation.x);
    ...
    ControlState controlRobot(){
        ...
        if(type_control_ == "approach_shelf"){
            ...
            case ControlState::CONTROL_APPROACH:
                ...
                {   
                    control_state = ControlState::CONTROL_DIRECTION;
                    robotStop();
                ...
                msg_cmd_vel.linear.x = saturateVel(msg_cmd_vel.linear.x,vel_min_linear_x_,vel_max_linear_x_ );
                msg_cmd_vel.angular.z = saturateVel(msg_cmd_vel.angular.z,vel_min_angular_z_,vel_max_angular_z_ );
                pub_cmd_vel_->publish(msg_cmd_vel);

                break;
                
            case ControlState::CONTROL_DIRECTION:
                getTransform("robot_base_link","target_goal_frame");
                calculateErrors();
            ...
            case ControlState::ENTER_TO_SHELF:
                ...
                RCLCPP_INFO(this->get_logger(),"distance target [%.3f]",distance_target);
