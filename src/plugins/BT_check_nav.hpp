...
class CheckNavGoal : public BT::SyncActionNode
{
    public:
        ...
        {
            return{ 
                BT::InputPort<std::string>("target_frame_check"),
        ...
        void checkGoal() {
            ...
            while(t.transform.translation.z  == std::numeric_limits<double>::max() && rclcpp::ok()){
                try {
                    ...
                    t = tf_buffer_->lookupTransform(target_frame_check_, "robot_base_link",tf2::TimePointZero);
            ...
            float error_nav = std::sqrt(std::pow(x_current,2)+ std::pow(y_current,2));
