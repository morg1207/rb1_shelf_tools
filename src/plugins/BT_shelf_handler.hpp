...
public:
  ShelfHandler(const std::string &name, const BT::NodeConfiguration &config)
    ...
    pub_shelf_down_ =
        node_->create_publisher<std_msgs::msg::String>("/elevator_down", 10);
  }
  ...
  virtual BT::NodeStatus tick() override {
    ...
    RCLCPP_INFO(node_->get_logger(), "Handling shelf action: %s",
                shelf_action.c_str());
    ...
    if (shelf_action == "up") {
      ...
      RCLCPP_ERROR(node_->get_logger(), "Unknown action: %s",
                   shelf_action.c_str());
