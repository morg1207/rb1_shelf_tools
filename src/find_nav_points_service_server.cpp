...
public:
  FindPointForNav() : Node("find_point_for_nav") {
    ...
    distance_to_shelf_for_find_pose_ = this->get_parameter("distance_to_shelf_for_find_pose").as_double();
...
private:
  ...
  // services callback
  void
  FindPointsForNavCallback(const std::shared_ptr<findNavPoses::Request> request,
                    const std::shared_ptr<findNavPoses::Response> response) {
    RCLCPP_DEBUG(this->get_logger(),
                 "-------------------------------------------");
    RCLCPP_DEBUG(this->get_logger(), "Find point nav server for shelf_position x  [%.3f]  y [%.3f]",
  ...
  float calculate_points(){
    ...
    float modulo = sqrt(std::pow(shelf_pose_y_ - t.transform.translation.y, 2) + std::pow(shelf_pose_x_ - t.transform.translation.x, 2));
    RCLCPP_INFO(this->get_logger(), "Modulo  [%.3f]", modulo);
    //hallo los vectores unitarios
    // vectores unitarios a normal
    RCLCPP_INFO(this->get_logger(), "Robot position  x [%.3f] y [%.3f]", t.transform.translation.x,t.transform.translation.y);
    RCLCPP_INFO(this->get_logger(), "Shelf position  x [%.3f] y [%.3f]", shelf_pose_x_,shelf_pose_y_);
    ...
    RCLCPP_INFO(this->get_logger(), "Punto 1 de navegacion  (%.3f, %.3f)", point_1_x, point_1_y);
    ...
    RCLCPP_INFO(this->get_logger(), "Punto 2 de navegacion  (%.3f, %.3f)", point_2_x, point_2_y);
  ...
  void verifyWithCostMap(float goal_x, float goal_y){
    ...
    if (isReachable(costmap_, goal_x, goal_y)) {
        RCLCPP_INFO(this->get_logger(), "Goal is reachable at (%.3f, %.3f)", goal_x, goal_y);
    } else {
        //auto [new_x, new_y] = findNearestReachablePoint(costmap_, goal_x, goal_y);
        //RCLCPP_WARN(this->get_logger(), "Goal not reachable, new goal at (%.3f, %.3f)", new_x, new_y);
    }

  }

    bool isReachable(const nav_msgs::msg::OccupancyGrid::SharedPtr &costmap, int x, int y) {
  ...
  indexMap convert_pose_to_map(float goal_x, float goal_y){
    ...
    RCLCPP_INFO(this->get_logger(), "Punto de navegacion index in the map (%d, %d)", index_map.x_index, index_map.y_index );
    return index_map;
